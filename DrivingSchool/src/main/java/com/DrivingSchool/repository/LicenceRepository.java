...
public interface LicenceRepository extends JpaRepository<Licence, Integer>{
	...
	@Query(value = "SELECT * FROM LICENCE WHERE CATEGORY_ID=?1 AND LICENCE_TYPE=?2 AND INSTRUCTOR_ID = ?3", nativeQuery = true)
	public Licence findLicenceByCategoryAndType(Integer categoryId, Integer licenceType, String instructorEmail);
	@Query(value = "SELECT * FROM LICENCE WHERE CATEGORY_ID=?1 AND LICENCE_TYPE=?2", nativeQuery = true)
	public List<Licence> findLicenceByCategoryAndType(Integer categoryId, Integer licenceType);
